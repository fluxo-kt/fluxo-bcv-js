...
private fun Project.configureTarget(
    ...
    val linkTasks: Provider<Set<KotlinJsIrLink>> =
        provider { linkTasksCollection + linkTasksFromBinaries }
...
private fun Project.configureKotlinCompilation(
    ...
    val buildDir = layout.buildDirectory.dir(targetConfig.apiDirName)
    ...
    val apiBuildTask = project.task<KotlinTsApiBuildTask>(buildTaskName) {
        ...
        generatedDefinitions.from(tsDefinitionFiles)
        dependsOn(linkTasks)
...
private fun Project.configureCheckTasks(
    ...
    fun configureApiCheckTask(t: DefaultTask) {
        t.isEnabled = apiCheckEnabled(t.project.name, state.bcv)
            && apiBuildTask.map { it.enabled }.getOrElse(true)
        t.group = VERIFICATION_GROUP
        t.description = "Checks signatures of public TypeScript API against the " +
            "golden value in API folder for :${t.project.name}"
        t.dependsOn(apiBuildTask)
    }

    val referenceFile = apiCheckDir.zip(buildFile) {
        dir, file -> dir.file(file.asFile.name)
    }

    fun registerCustomApiCheckTask() = task<KotlinTsApiCompareTask>(apiCheckTaskName) {
        configureApiCheckTask(this)
        compareApiDumps(referenceFile = referenceFile.get().asFile, buildFile = buildFile)
    }
